      ...
      <AnimatePresence mode="wait">
        {isIntro ? (
          <motion.div 
            ...
            exit={{ 
              opacity: 0, 
              transition: { duration: 1.2, ease: CUBIC_BEZIER_TRANSITION }
            }}
            className="absolute inset-0"
          >
            {/* 3D TUNNEL BACKGROUND - ONLY FOR INTRO */}
          ...
          >
            {/* SUBTLE DASHBOARD BACKGROUND (Clean technical look) */}
            <div className="absolute inset-0 z-0">
               <motion.div 
                 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] rounded-full opacity-[0.04]"
                 style={{ background: "radial-gradient(circle, #16FF00 0%, transparent 70%)" }}
                 animate={{ scale: [1, 1.1, 1], opacity: [0.03, 0.05, 0.03] }}
                 transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
               />
               <div className="absolute inset-0 opacity-[0.03] bg-[linear-gradient(rgba(22,255,0,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(22,255,0,0.1)_1px,transparent_1px)] bg-[size:50px_50px]" />
            </div>

            {/* DASHBOARD CONTENT */}
            <div className="relative z-10 grid min-h-screen max-w-7xl mx-auto items-center lg:grid-cols-2 px-10 gap-16">
