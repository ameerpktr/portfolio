        ...
        <div className="space-y-10 pl-8 sm:pl-10">
          {experiences.map((item, index) => (
            <motion.article
              ...
              className="relative"
            >
              <div className="absolute -left-[2.2rem] sm:-left-[2.7rem] top-8 h-3 w-3 rounded-full border-2 border-background bg-primary shadow-[0_0_0_6px_hsl(var(--primary)/0.12)]" />
              <div className="glass group rounded-xl p-5 sm:p-8 transition duration-300 hover:border-primary/40">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-extrabold text-primary">{item.year}</p>
                    <h3 className="mt-3 font-display text-xl font-extrabold text-foreground sm:text-2xl">{item.role}</h3>
                    <p className="text-sm font-semibold text-muted">{item.company}</p>
                  </div>
                </div>
                <div className="mt-8 grid gap-6 sm:mt-10 sm:grid-cols-2 sm:gap-10">
                  <div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-3 text-sm text-muted">
                      {item.wins.map((win) => (
                        <li key={win} className="rounded-xl border border-border bg-card-soft/70 p-3 sm:p-4 text-foreground/85 dark:bg-white/[0.03]">
                          {win}
                        </li>
                      ))}
                    </ul>
