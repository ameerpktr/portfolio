    ...
    <Section id="about" eyebrow="About me" title="Fintech precision, operational impact.">
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-[1.1fr_1.4fr] lg:gap-12">
        <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 24 }} viewport={{ once: true }}>
          <p className="max-w-xl text-justify text-sm sm:text-base font-medium leading-7 sm:leading-8 text-muted">
          ...
          <p className="mt-6 max-w-xl text-justify text-sm sm:text-base font-medium leading-7 sm:leading-8 text-muted">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-6 lg:grid-cols-3">
          {stats.map((stat, index) => (
            ...
            >
              <stat.icon className="mb-3 sm:mb-6 h-5 w-5 sm:h-6 sm:w-6 text-primary" />
              <p className="font-display text-xl sm:text-3xl font-extrabold text-foreground">{stat.value}</p>
              <p className="mt-1 sm:mt-2 text-[10px] sm:text-sm font-semibold leading-tight sm:leading-5 text-muted uppercase tracking-wider sm:normal-case sm:tracking-normal">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </div>
